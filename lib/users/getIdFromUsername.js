...
function getIdFromUsername(usernames) {
  ...
  const httpOpt = {
    url: "https://users.rotunnel.com/v1/usernames/users",
    options: {
      method: "POST",
      json: {
        usernames,
  ...
  return http(httpOpt).then(function (body) {
...
exports.func = function (args) {
  const usernames = Array.isArray(args.username)
    ? args.username
    : [args.username]; // cast usernames to array if necessary
